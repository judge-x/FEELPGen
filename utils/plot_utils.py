...
def plot_final_result(args,acces,losses):
    dataset_ = args.dataset.split('-')
    sub_dir = dataset_[0] + "/" + dataset_[2]  # e.g. Mnist/ratio0.5
    os.system("mkdir -p figs/{}".format(sub_dir))  # e.g. figs/Mnist/ratio0.5

    x_values=[i for i in range(len(acces))]
    # 绘制折线子图
    plt.figure(figsize=(10, 6))  # 设置总图的大小
    plt.subplot(2, 1, 1)  # 子图布局，2行1列，当前为第1个子图
    plt.plot(x_values, acces, marker='o', label='Line 1', color='b')
    plt.xlabel('Accuracy')
    plt.ylabel('Round')
    plt.title('Accuracy in Each Total Round')
    plt.legend()

    plt.subplot(2, 1, 2)  # 子图布局，2行1列，当前为第2个子图
    plt.plot(x_values, losses, marker='x', label='Line 2', color='r')
    plt.xlabel('Loss')
    plt.ylabel('Round')
    plt.title('Loss in Each Total Round')
    plt.legend()

    # 生成总图
    plt.tight_layout()  # 调整子图之间的间距，防止重叠
    plt.show()
