...
def get_dataset(mode='train', split='balanced'):
    ...
    dataset = EMNIST(root='./data', split=split, train=True if mode=='train' else False, download=False, transform=transform)
...
def main():
    ...
    parser.add_argument("--alpha", type=float, default=0.001, help="alpha in Dirichelt distribution (smaller means larger heterogeneity)")
    ...
    for i in range(NUM_SERVERS):
        ...
        path_prefix = f'u{args.n_user}-{args.split}-alpha{args.alpha}-ratio{args.sampling_ratio}-domain{i}'
