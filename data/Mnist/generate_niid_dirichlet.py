...
def get_dataset(mode='train'):
    ...
    # dataset_, _ = torch.utils.data.random_split(dataset, [n_sample, len(dataset) - n_sample])
...
def main():
    ...
    parser.add_argument("--alpha", type=float, default=0.02, help="alpha in Dirichelt distribution (smaller means larger heterogeneity)")
    parser.add_argument("--n_user", type=int, default=20,
                        help="number of local clients, should be muitiple of 10.")
    parser.add_argument("--n_domain",type=int,default=10,help="means the number of domain,equal to the number of servers")
    ...
    for i in range(NUM_DOMAINS):
        path_prefix = f'u{args.n_user}c{args.n_class}-alpha{args.alpha}-ratio{args.sampling_ratio}-domain{i}'
