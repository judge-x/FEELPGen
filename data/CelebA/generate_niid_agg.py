...
def process_data(domain_idx):
    ...
    def process_(mode, tf, ratio=0.1, user_lists=None, agg_user=-1):
        ...
        if agg_user > 0:
            dump_path=os.path.join(DUMP_PATH, 'user{}-agg{}-domain{}'.format(n_users, agg_user,domain_idx))
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agg_user", type=int, default=100, help="number of celebrities to be aggregated together as a device/client (as meta-batch size).")
    parser.add_argument("--ratio", type=float, default=2000, help="Number of total celebrities to be sampled for FL training.")
    parser.add_argument("--n_domain", type=int, default=10, help="Number of total celebrities to be sampled for FL training.")
