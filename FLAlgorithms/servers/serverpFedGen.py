...
class FedGen(Server):
    def __init__(self, args, model, seed, gpu_idx,ser_idx):
        super().__init__(args, model, seed, gpu_idx)
        ...
        print(self.device)
        self.student_model = copy.deepcopy(self.model).to(self.device)
        self.generative_model = create_generative_model(args.dataset, args.algorithm, self.model_name,
                                                        args.embedding,device=self.device).to(self.device)
        if not args.train:
        ...
        self.generative_optimizer = torch.optim.Adam(
            ...
            lr=args.gen_lr, betas=(0.9, 0.999),
        ...
        ## create the dymatic queue Q
        self.DL = [0 for _ in range(args.n_server)]
        self.tau = [0 for _ in range(args.n_server)]

        #### creating users ####
        self.users = []
        self.trainning_times = []
        for i in range(total_users):
            ...
            user = UserpFedGen(
                ...
                use_adam=self.use_adam,device=self.device)
            self.users.append(user)
            # self.trainning_times.append(user.training_time)
        ...
        print("Finished creating FedGen server {}.".format(ser_idx))
        self.max_trainning_time = random.uniform(1, 10)
    ...
    def train_generator(self, batch_size, epoches=1, latent_layer_idx=-1, verbose=False):
        ...
        def update_generator_(n_iters, student_model, TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS):
            ...
            for i in range(n_iters):
                ...
                # summary(self.generative_model, input_size=(1,1,32), device="cuda")
                ## feed to generator
    ...
    def train_global_model(self, local_epochs=20, batchsize=32):
        ...
        for epoch in range(local_epochs):
            optimizer.zero_grad()
            # random sample label
            sampled_y = np.random.choice(self.available_labels, batchsize)
            ...
            #     loss += nn.functional.cross_entropy(user_output_logp[item], sampled_y[item])
            loss = nn.functional.cross_entropy(user_output_logp, sampled_y)
            loss_ = loss / batchsize + 0.1 * teacher_loss
            loss_.backward()
            optimizer.step()
