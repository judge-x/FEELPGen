...
class FedProx(Server):
    def __init__(self, args, model,  seed, device, ser_idx):
        ...
        super().__init__(args, model, seed, device)#dataset, algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters,
                         #local_epochs, num_users, times)

        # Initialize data for all  users
        data = read_data(args.dataset,ser_idx=ser_idx)
        total_users = len(data[0])
        print("Users in total: {}".format(total_users))

        for i in range(total_users):
            id, train_data , test_data = read_user_data(i, data, dataset=args.dataset)
            user = UserFedProx(args, id, model, train_data, test_data, use_adam=False,device=self.device)
