...
class UserSD(User):
    def __init__(self,  args, id, model, train_data, test_data, use_adam=False, device="cpu"):
        device_=device
        super().__init__(args, id, model, train_data, test_data, use_adam=use_adam, device=device_)
    
        #init training time which random generate  in [1,20]
        max=random.randint(10,20)
        self.training_time=random.randint(1,max)
    ...
    def train(self, glob_iter, personalized=False, lr_decay=True, count_labels=True):
        ...
        for epoch in range(1, self.local_epochs + 1):
            ...
            for i in range(self.K):
                ...
                X, y = result['X'].to(self.device), result['y'].to(self.device)
                if count_labels:
