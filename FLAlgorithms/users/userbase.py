...
class User:
    ...
    """
    def __init__(
            self, args, id, model, train_data, test_data, use_adam=False,device="cpu"):
        self.device=torch.device(device if args.device=='cuda' else "cpu")
        self.model = copy.deepcopy(model[0]).to(self.device)
        ...
        self.trainloader = DataLoader(train_data, self.batch_size, shuffle=True, drop_last=False)
